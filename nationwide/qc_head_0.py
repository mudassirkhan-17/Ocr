...
class PolicyPageExtractor:
    ...
    def merge_page_ranges(self, pages: List[int], buffer: int = 0) -> List[Tuple[int, int]]:
        """
    ...
    def find_headings(self) -> Dict[str, List[HeadingMatch]]:
        ...
        merged_ranges = self.merge_page_ranges(dollar_pages, buffer=0)
...
def save_results(all_results: Dict):
    ...
    output_file = Path("encovaop/salem_extraction2.json")
    txt_file = Path("encovaop/salem_extraction2.txt")
...
def save_results(all_results: Dict, base_filename: str = None):
    ...
    if base_filename:
        ...
        output_file = Path(f"nationwideop/{carrier_name}_extraction0.json")
        txt_file = Path(f"nationwideop/{carrier_name}_extraction0.txt")
    else:
        # Fallback to default names - save to encovaop folder
        # Path("encovaop").mkdir(exist_ok=True)
        output_file = Path("nationwideop/evergreen_extraction0.json")
        txt_file = Path("nationwideop/evergreen_extraction0.txt")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        files_to_test = [
            ...
            "nationwideop/evergreen_policy2.txt",            # shorter (NEW)
            # OLD OCR OUTPUTS (for comparison)
            # "divine_cna_Package_ocr_output2.txt",          # CNA (OLD - 300 DPI no preprocess)
            # "divine_cna_Package_ocr_output.txt",           # CNA (OLDEST - 200 DPI)
