...
def combine_extraction_files(
    ...
    if output_file is None:
        ...
        output_file = Path("nationwideop/evergreen_combined.txt")
    else:
        # Save to encovaop folder
        # Path("encovaop").mkdir(exist_ok=True)
        output_file = Path(f"nationwideop/{output_file}")
...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        default_tesseract = "nationwideop/evergreen_extraction1.txt"  # Tesseract with buffer=1
        default_pymupdf = "nationwideop/evergreen_extraction0.txt"  # PyMuPDF with buffer=0
