...
def combine_extraction_files(
    ...
    if output_file is None:
        ...
        output_file = Path("travelerop/jay_combined.txt")
    else:
        # Save to encovaop folder
        # Path("encovaop").mkdir(exist_ok=True)
        output_file = Path(f"travelerop/{output_file}")
...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        default_tesseract = "travelerop/jay_extraction1.txt"  # Tesseract with buffer=1
        default_pymupdf = "travelerop/jay_extraction0.txt"  # PyMuPDF with buffer=0
