...
def save_results(all_results: Dict, base_filename: str = None):
    ...
    if base_filename:
        ...
        output_file = Path(f"encovaop/{carrier_name}_extraction1.json")
        txt_file = Path(f"encovaop/{carrier_name}_extraction1.txt")
    else:
        # Fallback to default names - save to encovaop folder
        # Path("encovaop").mkdir(exist_ok=True)
        output_file = Path("encovaop/salem_extraction1.json")
        txt_file = Path("encovaop/salem_extraction1.txt")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        files_to_test = [
            ...
            "encovaop/salem_ocrp6.txt",            # Aaniya (NEW)
