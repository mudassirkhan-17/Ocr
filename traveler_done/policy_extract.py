...
Usage:
    ...
    python policy_extract.py encova/aaniya_policy.pdf --max-pages 25

Outputs:
    - encovaop/{base_name}_pol1.txt (Tesseract extraction)
    - encovaop/{base_name}_pol2.txt (PyMuPDF extraction)
...
def extract_base_name(input_path: str) -> str:
    ...
    Examples:
        ...
        "encova/aaniya_policy.pdf" -> "aaniya_policy"
    """
...
def find_pdf_file(base_name: str) -> Optional[Path]:
    ...
    Tries:
        ...
        2. traveler/{base_name}.pdf
        3. travelerop/{base_name}.pdf
        4. {base_name} (as-is, if it's already a full path)
    """
    candidates = [
        Path(f"{base_name}.pdf"),
        Path(f"traveler/{base_name}.pdf"),
        Path(f"travelerop/{base_name}.pdf"),
...
def extract_tesseract(pdf_path: Path, output_path: Path, n_jobs: int = -1, max_pages: Optional[int] = None) -> bool:
    ...
    print(f"Extracting text using Tesseract OCR ({n_jobs if n_jobs != -1 else 'all'} workers)...")
...
def main():
    ...
    if not pdf_path:
        ...
        print(f"     - traveler/{base_name}.pdf")
        print(f"     - travelerop/{base_name}.pdf")
        return
    
    print(f"✅ Found PDF: {pdf_path}\n")
    
    # Set up output paths
    output_dir = Path("travelerop")
