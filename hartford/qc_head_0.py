...
def save_results(all_results: Dict, base_filename: str = None):
    ...
    if base_filename:
        ...
        output_file = Path(f"hartfordop/{carrier_name}_extraction0.json")
        txt_file = Path(f"hartfordop/{carrier_name}_extraction0.txt")
    else:
        # Fallback to default names - save to encovaop folder
        # Path("encovaop").mkdir(exist_ok=True)
        output_file = Path("hartfordop/ameen_extraction0.json")
        txt_file = Path("hartfordop/ameen_extraction0.txt")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        files_to_test = [
            ...
            "hartfordop/ameen_policy2.txt",            # shorter (NEW)
