"""
Encova Certificate + Policy Extraction (Property + GL) with QC

- Uses GPT-4.1 Mini by default
- Accepts:
  - Encova combined policy text (Tesseract + PyMuPDF)
...
def _qc_compare(certificate: Dict[str, Any], policy: Dict[str, Any]) -> Dict[str, Any]:
    """
    Deterministic QC: compares certificate vs policy on key Encova fields.
    ...
    for field, cert_path, pol_path in checks:
        ...
        # Only flag mismatch if both present (certificate provided) and both non-nullish
        if cert_val is None or pol_val is None:
            continue
        if not _compare_values(cert_val, pol_val):
            mismatches.append({
                "field": field,
...
def _extract_first_class_amount(policy_text: str, class_no: int) -> Optional[str]:
    """
    Best-effort regex extraction for Encova declarations:
...
class EncovaExtractor:
    """Extract Encova certificate + policy data using an LLM."""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract Encova Property + GL from policy + optional certificates, then QC compare",
...
Examples:
  ...
  python llm4nano.py shelby
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="encova_extraction_results.json",
    ...
    try:
        ...
        if output_file == "encova_extraction_results.json":  # default value
