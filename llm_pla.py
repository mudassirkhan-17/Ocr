...
class CertificateExtractor:
    ...
    def create_extraction_prompt(self, pdfplumber_text: str, pymupdf_text: str = None, tesseract_text: str = None) -> str:
        ...
        """
        has_multiple_sources = (pymupdf_text or tesseract_text)
        
        if has_multiple_sources:
            # Triple extraction mode - cross-validation (ACORD PL)
...
**IMPORTANT: You have MULTIPLE extraction sources for the SAME document below.**

**EXTRACTION SOURCES (in priority order):**
1. **PDFPLUMBER (Table-aware)**: Preserves table structure - USE THIS AS PRIMARY SOURCE for coverage data and structured information
2. **PYMUPDF (Text layer)**: Raw text extraction - use as cross-validation/fallback
3. **TESSERACT (OCR)**: OCR extraction - use as additional fallback for scanned documents

**PRIORITY**: For coverage extraction and structured data, prioritize pdfplumber's TABLE sections.
Cross-validate with PyMuPDF and Tesseract text when needed, but trust pdfplumber's structured table data first.
...
Your task is to:
1. **Compare all extraction sources** to identify discrepancies
2. **Cross-validate** the information between all sources
3. **Choose the most accurate value** when there are conflicts (prioritize pdfplumber for structured data)
4. **Handle OCR errors** intelligently using context from all sources
5. **Fill gaps** where one source captured data the others missed
...
**Validation Rules:**
- If all sources agree → high confidence, use that value
- If sources disagree → prioritize pdfplumber for structured data (tables), use the one that makes more logical sense
- If one source has formatting issues → prefer the cleaner formatted value (prioritize pdfplumber)
...
==================================================
**PRIORITY**: Use the TABLE sections for coverage data extraction.
...
""" + (pymupdf_text if pymupdf_text else "[PyMuPDF extraction not available]") + """

""" + (f"""
==================================================
EXTRACTION SOURCE 3: TESSERACT (OCR) - ADDITIONAL FALLBACK
==================================================
**USE AS**: Additional fallback for scanned documents or when other sources fail.

""" + tesseract_text + """
""" if tesseract_text else "") + """

Now analyze all sources, cross-validate, and return the JSON object:"""
        else:
